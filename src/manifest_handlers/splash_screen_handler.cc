...
const char kSplashScreenReadyWhenKey[] = "widget.launch_screen";
...
void ConvertStringToColor(const std::string& color_str,
                          wgt_parse::Color* color) {
...
bool SplashScreenHandler::ParseSingleOrientation(
  ...
  auto dict_element_parser =
    ...
    if (!parser::VerifyElementNamespace(*dict, keys::kTizenNamespacePrefix))
      return false;
    if (!ParseColor(dict, &splash_screen)) return false;
    if (!ParseElement(dict, &splash_screen.background_image,
                      kSplashScreenBgImage))
      return false;
    if (!ParseElement(dict, &splash_screen.image, kSplashScreenImage))
      return false;
    if (!ParseElement(dict, &splash_screen.image_border,
                      kSplashScreenImageBorder))
      return false;
    return true;
  };
  ...
  std::vector<const parser::DictionaryValue*> values_to_parse;

  switch (val->GetType()) {
    case parser::Value::Type::TYPE_DICTIONARY: {
      const parser::DictionaryValue* dict = nullptr;
      val->GetAsDictionary(&dict);
      values_to_parse.push_back(dict);
    } break;
    case parser::Value::Type::TYPE_LIST: {
      const parser::ListValue* list = nullptr;
      val->GetAsList(&list);
      for (const auto& list_element : *list) {
        if (list_element->GetType() == parser::Value::Type::TYPE_DICTIONARY) {
          const parser::DictionaryValue* dict = nullptr;
          val->GetAsDictionary(&dict);
          values_to_parse.push_back(dict);
        }
  ...
  for (auto& dict_element : values_to_parse) {
    valid_element = dict_element_parser(splash_screen, dict_element);
...
bool SplashScreenHandler::ParseColor(const parser::DictionaryValue* dict,
  ...
  ConvertStringToColor(only_hex, &splash_screen->color);
  return true;
}
...
bool SplashScreenHandler::ParseReadyWhen(const parser::Manifest& manifest,
                                         SplashScreenInfo* ss_info) {
  const parser::Value* val = nullptr;
  manifest.Get(kSplashScreenReadyWhenKey, &val);

  std::string ready_when;

  switch (val->GetType()) {
    case parser::Value::TYPE_DICTIONARY: {
      const parser::DictionaryValue* dict = nullptr;
      val->GetAsDictionary(&dict);
      dict->GetString(kSplashScreenReadyWhen, &ready_when);
    } break;
    case parser::Value::TYPE_LIST: {
      const parser::ListValue* list = nullptr;
      val->GetAsList(&list);
      for (const auto& list_element : *list) {
        if (list_element->GetType() == parser::Value::Type::TYPE_DICTIONARY) {
          const parser::DictionaryValue* dict = nullptr;
          val->GetAsDictionary(&dict);
          if (dict->GetString(kSplashScreenReadyWhen, &ready_when)) break;
        }
      }
    } break;
  }
  ...
  ready_when_functors[kUserInteractive] = std::bind(
      &SplashScreenInfo::set_ready_when, ss_info, ReadyWhen::USERINTERACTIVE);
...
bool SplashScreenHandler::Parse(const parser::Manifest& manifest,
                                ...
                                std::string* error) {
  std::shared_ptr<SplashScreenInfo> ss_info =
      std::make_shared<SplashScreenInfo>();
...
std::string SplashScreenHandler::Key() const { return keys::kSplashScreen; }
