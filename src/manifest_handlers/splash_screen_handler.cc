...
void ConvertStringToColor(const std::string& color_str, Color* color) {
...
bool SplashScreenHandler::ParseSingleOrientation(
  ...
  if (!std::any_of(values_to_parse.begin(), values_to_parse.end(),
                   std::bind(dict_element_parser, std::ref(splash_screen),
                             std::placeholders::_1)))
    return false;
