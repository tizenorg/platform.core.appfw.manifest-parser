...
class SplashScreenInfo : public parser::ManifestData {
 public:
  ...
  void set_splash_screen_data(std::pair<ScreenOrientation,
                              SplashScreenData> splash_screen_data) {
    splash_screen_data_.insert(splash_screen_data);
  }
  const std::map<ScreenOrientation, SplashScreenData>&
  splash_screen_data() const {
    return splash_screen_data_;
  }
  void set_ready_when(ReadyWhen ready_when) {
    ready_when_ = ready_when;
  }
  const ReadyWhen ready_when() const {
    return ready_when_;
  }
...
class SplashScreenHandler : public parser::ManifestHandler {
 ...
 private:
  ...
  void ToColor(const std::string& color_str, Color* color);
};
