...
void WidgetHandler::ParseAuthorElements(const parser::Manifest& manifest,
  ...
  if (manifest.HasPath(keys::kAuthorKey)) {
    ...
    if (author_value->GetType() == parser::Value::TYPE_DICTIONARY) {
      ...
      if (!parser::VerifyElementNamespace(*author_dict, kWidgetNamespacePrefix))
        return;
...
bool WidgetHandler::Parse(const parser::Manifest& manifest,
  ...
  if (manifest.HasPath(kPreferencesKey)) {
    const parser::Value* val = nullptr;
    if (manifest.Get(kPreferencesKey, &val)) {
      if (val->GetType() == parser::Value::TYPE_LIST) {
        ...
        if (val->GetAsList(&pref_list)) {
          ...
          for (const auto& pref : *pref_list) {
            ...
            if (!ParserPreferenceItem(pref, &preference, error)) return false;
        ...
        // only one preference
        Preference* pref;
        if (!ParserPreferenceItem(val, &pref, error)) return false;
