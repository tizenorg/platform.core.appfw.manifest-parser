...
bool AccountHandler::Parse(const parser::Manifest& manifest,
  ...
  if (manifest.Get(keys::kAccountKey, &val)) {
    if (manifest.Get(keys::kAccountProviderKey, &val)) {
...
bool AccountHandler::ParseSingleAccountElement(
  ...
  std::string app_id;
  if (!item_dict->GetString(keys::kAccountAppIDKey, &app_id)) {
  ...
  if (!item_dict->GetString(keys::kAccountProviderIDKey, &provider_id)) {
  ...
  if (!ParseLabels(item_dict, &single_account)) {
