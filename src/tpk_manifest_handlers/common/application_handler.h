...
bool ParseAppControl(const parser::DictionaryValue& dict,
                     T* info, std::string*) {
  for (const auto& itemOperation : parser::GetOneOrMany(&dict,
      tpk_app_keys::kAppControlOperationKey, "")) {
    std::string operation;
    std::string uri;
    std::string mime;
    itemOperation->GetString(tpk_app_keys::kAppControlNameChildKey, &operation);

    for (const auto& itemURI : parser::GetOneOrMany(&dict,
        tpk_app_keys::kAppControlURIKey, "")) {
      itemURI->GetString(tpk_app_keys::kAppControlNameChildKey, &uri);

      for (const auto& itemMime : parser::GetOneOrMany(&dict,
          tpk_app_keys::kAppControlMimeKey, "")) {
        itemMime->GetString(tpk_app_keys::kAppControlNameChildKey, &mime);
...
bool AppControlValidation(const T& it, std::string* error) {
  for (const auto& item : it.app_control) {
    ...
    if (!item.uri().empty()) {
      *error = "The uri child element of app-control element is obligatory";
      return false;
    }
    if (item.mime().empty()) {
      *error = "The mime child element of app-control element is obligatory";
      return false;
    }
  }
  return true;
