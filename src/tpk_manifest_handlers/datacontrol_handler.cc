...
void ParseDataControlAndStore(
  ...
  if (control_dict.GetDictionary(keys::kDataControlProviderIDKey,
                                 &providerid_dict)) {
  ...
  if (control_dict.GetDictionary(keys::kDataControlTypeKey,
                                 &type_dict)) {
    type_dict->GetString(
  ...
  if (!manifest.Get(keys::kDataControlKey, &value))
    ...
    for (const auto& item : *list) {
      ...
      if (!item->GetAsDictionary(&control_dict)) {
        *error = "Parsing datacontrol element failed";
        return false;
      }

      ParseDataControlAndStore(*control_dict, datacontrolinfo.get());
    }
