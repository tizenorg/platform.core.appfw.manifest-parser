...
bool InitializeParsing(const parser::DictionaryValue& app_dict,
                       ...
                       std::string* error) {
  if (!InitializeParsingElement(app_dict, app_keys::kAppControlKey,
      ParseAppControl, uiapplicationinfo, error))
    return false;
  if (!InitializeParsingElement(app_dict, app_keys::kDataControlKey,
      ParseDataControl, uiapplicationinfo, error))
    return false;
  if (!InitializeParsingElement(app_dict, app_keys::kMetaDataKey,
      ParseMetaData, uiapplicationinfo, error))
    return false;
  if (!InitializeParsingElement(app_dict, app_keys::kIconKey,
      ParseAppIcon, uiapplicationinfo, error))
    return false;
  if (!InitializeParsingElement(app_dict, app_keys::kLabelKey,
      ParseLabel, uiapplicationinfo, error))
    return false;
  if (!InitializeParsingElement(app_dict, kImageKey,
      ParseAppImage, uiapplicationinfo, error))
    return false;
  if (!InitializeParsingElement(app_dict, app_keys::kBackgroundCategoryKey,
      ParseBackgroundCategoryElement, uiapplicationinfo, error))
    return false;
