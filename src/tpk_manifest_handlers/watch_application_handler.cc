...
bool WatchAppValidation(const WatchApplicationInfo& watch_app_info,
  ...
  if (watch_app_info.ambient_support().empty()) {
    *error = "The ambient element of watch-application element is obligatory";
    return false;
  }

  if (watch_app_info.label().empty()) {
    *error = "The label element of watch-application element is obligatory";
    return false;
  }

  if (watch_app_info.icon().empty()) {
    *error = "The icon element of watch-application element is obligatory";
    return false;
  }
...
bool WatchApplicationHandler::Parse(
  ...
  auto watcha_app_info = std::make_shared<WatchApplicationInfo>();

  for (const auto& watch_app_dict :
      parser::GetOneOrMany(manifest.value(), kWatchApplicationKey, "")) {
    if (!ParseWatchApplication(*watch_app_dict, watcha_app_info.get(), error))
      return false;
  }

  *output = std::static_pointer_cast<parser::ManifestData>(watcha_app_info);
  return true;
}
